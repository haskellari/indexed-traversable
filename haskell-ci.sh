...
PKGDIR_with_index="$(find "$BUILDDIR/unpacked" -maxdepth 1 -type d -regex '.*/with-index-[0-9.]*')"
PKGDIR_with_index_instances="$(find "$BUILDDIR/unpacked" -maxdepth 1 -type d -regex '.*/with-index-instances-[0-9.]*')"
run_cmd touch cabal.project
run_cmd touch cabal.project.local
echo_to cabal.project "packages: ${PKGDIR_with_index}"
echo_to cabal.project "packages: ${PKGDIR_with_index_instances}"
echo_if_to $((HCNUMVER >= 80200)) cabal.project "package with-index"
echo_if_to $((HCNUMVER >= 80200)) cabal.project "    ghc-options: -Werror=missing-methods"
echo_if_to $((HCNUMVER >= 80200)) cabal.project "package with-index-instances"
echo_if_to $((HCNUMVER >= 80200)) cabal.project "    ghc-options: -Werror=missing-methods"
cat >> cabal.project <<EOF
EOF
$HCPKG list --simple-output --names-only | perl -ne 'for (split /\s+/) { print "constraints: $_ installed\n" unless /^(with-index|with-index-instances)$/; }' >> cabal.project.local
...
change_dir "${PKGDIR_with_index}"
run_cmd ${CABAL} -vnormal check
change_dir "${PKGDIR_with_index_instances}"
run_cmd ${CABAL} -vnormal check
change_dir "$BUILDDIR"
